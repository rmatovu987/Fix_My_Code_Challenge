...
if __name__ == '__main__':
    ...
    if not user_1.is_valid_password(u_pwd):
        raise ValueError("User.is_valid_password should return True if it's the right \
password")

    if user_1.is_valid_password("Fakepwd"):
        raise ValueError("is_valid_password should return False if it's not the right \
password")

    if user_1.is_valid_password(None):
        raise ValueError("is_valid_password should return False if compare with None")

    if user_1.is_valid_password(89):
        raise TypeError("is_valid_password should return False if compare with integer")

    if user_2.is_valid_password("No pwd"):
        raise ValueError("is_valid_password should return False if no password set \
before")
